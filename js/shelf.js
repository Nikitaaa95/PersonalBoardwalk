...
(function () {
  ...
  function buildSpine(item) {
    ...
    /* A photographed spine is the book itself, so none of the drawn furniture
       applies: no cloth, no bands, no set lettering. Its own proportions set
       its width against the height the shelf gives it, exactly as a scanned
       cover does on a face-out book, so the photograph is never stretched. */
    if (item.spineImage) {
      btn.classList.add("spine--image");
      btn.style.height = ((item.height || 0.94) * 100) + "%";
      btn.style.setProperty("--spine-aspect", item.spineAspect || 0.12);
      var shot = el("img");
      shot.src = item.spineImage;
      shot.alt = "";
